...
learning vehicle for distributed systems patterns, in public, one service at a time. A
developer connects a Git repo, pushes code, and Pallet builds it, deploys it, and hands back a
live URL with TLS. Underneath, it's an event-driven architecture: no service makes a blocking
call to another (the only synchronous edges are inbound webhooks/dashboard requests and calls to
third parties), the deploy and billing pipelines run as Temporal sagas, and every external call
goes through retries/timeouts/circuit breakers. Full design brief: `PROJECT.md`. Decisions and
...
`PersistenceExceptionHandler` win first) renders it via `ErrorResponse` automatically, with zero
changes needed to the handler itself. Don't add a new `@ExceptionHandler` for a domain exception;
extend `AppException` instead. An unmapped, non-`AppException` throwable reaches the client as a
raw 500 via the `Exception.class` fallback: that's a bug to fix, not an acceptable gap.
...
circuit-breaker/retry/time-limiter policy and either rethrows an `AppException` untouched or wraps
...
Never mock Postgres, Kafka, Redis, or Keycloak where a container is standing in for them: that's
the whole point of paying the container startup cost. Assert on `ApiResponse`/
`PageResponse`/`ErrorResponse` via `PalletAssertions.assertThat(...)`, not hand-written `jsonPath`
chains. Containers are shared/singleton across a test run, so don't add a second `@Testcontainers`
setup; import the shared configuration instead.
...
- Don't add `platform-common` as a reactor sibling of a service, or otherwise let a service resolve
  ...
  (`PACKAGES.md`).
- Don't commit a real secret to any tracked `application*.yaml`/`config-repo/*.yml`. Use env vars or
...
- **Never mention AI, an AI tool, or this being AI-generated/assisted in a comment, Javadoc, or
  commit message.** No "auto-generated", "as an AI", tool attributions, or similar. Comments should
  read as if a human on the team wrote them, because they should have been.
