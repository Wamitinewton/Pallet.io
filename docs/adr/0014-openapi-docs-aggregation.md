...
The straightforward way to document one service is springdoc (`springdoc-openapi-starter-webmvc-api`)
generating the spec, with a self-hosted, vendored `scalar.js` bundle rendering it as a static
`internal-docs/index.html`. That works fine for a single monolith: one `openapi.json`, one page
pointing at it.
...
glob, resolved from `pallet.gateway.routes.<service>` (`config-repo/api-gateway.yml`). Copying the
single-service pattern verbatim into every service, one `OpenApiConfig` and one vendored Scalar
page each, would mean a developer opens `identity-service`'s docs at one URL and
`notification-service`'s at another, with every new service repeating the same `OpenApiConfig`
boilerplate (bearer scheme, `Info` block, error-schema wiring) that
`platform-common-exception`/`-api` already centralized for everything else those services return.
...
- `PalletOpenApiAutoConfiguration` registers the `bearerAuth` HTTP/JWT security scheme once
  (replacing every service's own copy of that `OpenApiConfig` security-scheme block) and a
...
- Deliberately **not** in scope: springdoc already reflects a controller's real generic return
  type (`ResponseEntity<ApiResponse<UserDto>>`) into the correct nested schema on its own, so no
...
is needed for a service's docs to become reachable, since it's already covered by the existing
...
current product decision. `scalar.js` is vendored as a gateway static asset, matching the
project's general skepticism of unnecessary external runtime dependencies (see
`docs/workflows/api-gateway/07-docs-hub.md`).

A service becomes visible in the docs hub **the moment it's a `pallet.gateway.routes` entry**: the
same act that makes it reachable at all. There is no second "docs registry" to keep in sync with
the routing table.
...
matching the "just annotations" bar directly. The docs hub needs zero code change when a new
...
gateway at request time. If a backend is down, that service's tab fails to load in the hub (the
...
`pallet.gateway.routes` for them). That's worth revisiting if one of them later grows a real
...
- **Build-time bundling into a static, published docs site** (Redocly/ReadMe-style). Rejected for
  ...
  opposite of what's useful while the API surface is still moving fast. Worth revisiting if Pallet
  ever ships a public, versioned, external-facing developer docs site, a different problem from
  "see every service's current API from one URL," which is what this ADR solves.
- **Each service keeps its own vendored Scalar page**, copied N times. Rejected: fails the stated
  goal directly. A developer would still open N URLs, and every service would carry the same
  duplicated `OpenApiConfig`/static-asset boilerplate `platform-common-openapi` exists to remove.
