 ...
 * and a genuine Keycloak connection failure. {@link OrgInviteRejectedListenerIntegrationTest}
 * covers the disable, mismatch and no-op paths against a real Keycloak.
 */
@UnitTest
class OrgInviteRejectedListenerTest {
    ...
    void guardShortCircuitSkipsKeycloakAndTheLocalRepository() {
        ...
        OrgInviteRejectedListener listener = new OrgInviteRejectedListener(
                idempotencyGuard,
                externalCall,
                mock(Keycloak.class),
        ...
        listener.onMessage(recordFor(OrgInviteRejected.of(
                "org-1", "invite-1", "kc-user-1", "user@example.test", OrgInviteRejected.REASON_REVOKED)));
    ...
    void keycloakFailureReleasesTheGuardAndRethrowsWithoutTouchingTheLocalRepository() throws IOException {
        ...
        verifyNoInteractions(identityUserRepository);
        verifyNoInteractions(auditPublisher);
        verifyNoInteractions(revokedSessionRegistry);
    }

    private ConsumerRecord<String, JsonNode> recordFor(OrgInviteRejected event) {
        return new ConsumerRecord<>("org.invite.rejected", 0, 0, event.orgId(), jsonMapper.valueToTree(event));
    }
