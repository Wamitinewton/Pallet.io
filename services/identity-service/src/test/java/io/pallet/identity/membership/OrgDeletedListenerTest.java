 ...
 * a genuine Keycloak connection failure. {@link OrgDeletedListenerIntegrationTest} covers the
 * success and no-op paths against a real Keycloak.
 */
@UnitTest
class OrgDeletedListenerTest {
    ...
    void guardShortCircuitSkipsKeycloakAndTheLocalRepository() {
        ...
        OrgDeletedListener listener = new OrgDeletedListener(
                idempotencyGuard,
                externalCall,
                mock(Keycloak.class),
        ...
        listener.onMessage(recordFor(OrgDeleted.of("org-1", "kc-user-owner")));
    ...
    @Test
    void keycloakFailureReleasesTheGuardAndRethrowsWithoutSavingAnything() throws IOException {
        when(idempotencyGuard.markProcessed(anyString(), any())).thenReturn(true);
        when(identityUserRepository.findByOrgId("org-1"))
                .thenReturn(List.of(new IdentityUser("org-1", "kc-user-1", "user@example.test", "Test User")));
        ...
        OrgDeletedListener listener = new OrgDeletedListener(
                ...
                jsonMapper,
                new SimpleMeterRegistry());
        OrgDeleted event = OrgDeleted.of("org-1", "kc-user-owner");
        ...
        verify(identityUserRepository, never()).saveAll(any());
    }

    private ConsumerRecord<String, JsonNode> recordFor(OrgDeleted event) {
        return new ConsumerRecord<>("org.deleted", 0, 0, event.orgId(), jsonMapper.valueToTree(event));
    }
